...
routeUtils.route = async (app, routes = []) => {
    routes.forEach(route => {
        let middlewares = [];
        if (route.joiSchemaForSwagger.file) {
            const multerMiddleware = getMulterMiddleware(route.joiSchemaForSwagger.file);
...
let getMulterMiddleware = (file) => {
    ...
    if (file && Object.keys(file).length) {
        const fileField = Object.keys(file)[0];
