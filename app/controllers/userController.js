...
userController.getDashboardDataAdmin = async (payload) => {
    ...
    return createSuccessResponse(MESSAGES.SERVER_IS_WORKING_FINE, {usersCount, referPoints: (data[0]?.refer[0]?.sum) ? (data[0]?.refer[0]?.sum) : 0, dailyPoints: (data[0]?.daily[0]?.sum) ? (data[0]?.daily[0]?.sum) : 0 });
};
...
userController.userSignup = async (payload) => {
    ...
    return createSuccessResponse(MESSAGES.USER_REGISTERED_SUCCESSFULLY, user);
};
...
userController.loginUser = async (payload) => {
    let user = await dbService.findOne(UserModel, { $or: [ { email: payload.id }, { phone: payload.id }, { userName: payload.id } ] });
