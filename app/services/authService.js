...
let validateUser = async (request, authType) => {
    try {
        ...
        if (authType == AVAILABLE_AUTHS.ADMIN_USER && (session.userType !== USER_TYPE.USER && session.userType !== USER_TYPE.ADMIN)) {
            return false;
        }
        
        if(authType == AVAILABLE_AUTHS.ADMIN && session.userType !== USER_TYPE.ADMIN){
            return false;
        }
